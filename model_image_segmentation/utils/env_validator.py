"""
环境验证器 - 检查PyTorch、MMSegmentation、CUDA等环境
"""
...
def validate_environment() -> Tuple[bool, str, Dict[str, Any]]:
    ...
    # 检查 MMSegmentation（必须）
    try:
        import mmseg
        info['mmseg_version'] = mmseg.__version__
        
        # 进一步检查MMSeg核心组件
        try:
            from mmseg.apis import init_model
            from mmseg.models import build_segmentor
            info['mmseg_apis_ok'] = True
        except ImportError as e:
            errors.append(f"❌ MMSegmentation 组件不完整: {e}")
            info['mmseg_apis_ok'] = False
    except ImportError:
        errors.append("❌ MMSegmentation 未安装（必须）\n   请运行: mim install mmsegmentation")
        info['mmseg_version'] = None
    
    # 检查 MMEngine（必须）
    try:
        import mmengine
        info['mmengine_version'] = mmengine.__version__
    except ImportError:
        errors.append("❌ MMEngine 未安装（必须）\n   请运行: mim install mmengine")
    
    # 检查 MMCV（必须）
    try:
        import mmcv
        info['mmcv_version'] = mmcv.__version__
    except ImportError:
        errors.append("❌ MMCV 未安装（必须）\n   请运行: mim install 'mmcv>=2.0.0'")
    ...
    if info.get('mmseg_version'):
        lines.append(f"   MMSegmentation: {info['mmseg_version']}")
    if info.get('mmengine_version'):
        lines.append(f"   MMEngine: {info['mmengine_version']}")
...
def get_recommended_batch_size(gpu_memory_gb: float, img_size: int = 512, model_scale: str = "中") -> int:
    """
    根据GPU显存和模型大小推荐批次大小
    ...
    """
    # 基础参考值 (512尺寸, 中型模型)
    base_memory_per_image = 1.0  # GB per image (SegFormer比YOLO大)
